-- "Caldaia" and "Condizionatore" added as selectable tipo_lavoro values, each with their own detail blob.

alter table pratiche
  add column if not exists dettaglio_caldaia jsonb,
  add column if not exists dettaglio_condizionatore jsonb;
...
language sql security definer set search_path = public as $$
  select
    ...
    p.dettaglio_schermature_solari, p.dettaglio_caldaia, p.dettaglio_condizionatore
  from pratiche p
