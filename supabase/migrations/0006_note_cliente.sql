-- A note field the end client can fill in themselves, separate from the admin-only "note" (internal notes).

alter table pratiche add column if not exists note_cliente text;
